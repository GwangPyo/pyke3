    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove('stock_analysis', 'fundamental_positive', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.is_fundamental_good: got unexpected plan from when clause 1"
            with engine.prove('stock_analysis', 'fundamental_positive', context,
                              (rule.pattern(0),
                               rule.pattern(2),)) \
              as gen_2:
              for x_2 in gen_2:
                assert x_2 is None, \
                  "investment_rules.is_fundamental_good: got unexpected plan from when clause 2"
                with engine.prove('stock_analysis', 'fundamental_positive', context,
                                  (rule.pattern(0),
                                   rule.pattern(3),)) \
                  as gen_3:
                  for x_3 in gen_3:
                    assert x_3 is None, \
                      "investment_rules.is_fundamental_good: got unexpected plan from when clause 3"
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove('stock_analysis', 'fundamental_negative', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.is_fundamental_bad: got unexpected plan from when clause 1"
            with engine.prove('stock_analysis', 'fundamental_negative', context,
                              (rule.pattern(0),
                               rule.pattern(2),)) \
              as gen_2:
              for x_2 in gen_2:
                assert x_2 is None, \
                  "investment_rules.is_fundamental_bad: got unexpected plan from when clause 2"
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove('stock_analysis', 'technical_indicator', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.is_technical_bullish: got unexpected plan from when clause 1"
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove('stock_analysis', 'technical_indicator', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.is_technical_bearish: got unexpected plan from when clause 1"
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove('stock_analysis', 'news_positive', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.is_news_positive: got unexpected plan from when clause 1"
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove('stock_analysis', 'news_negative', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.is_news_negative: got unexpected plan from when clause 1"
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove('stock_analysis', 'market_condition', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.is_market_bad: got unexpected plan from when clause 1"
            rule.rule_base.num_bc_rule_successes += 1
            yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'is_fundamental_good', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.buy_a_lot_rule: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'is_technical_bullish', context,
                              (rule.pattern(0),)) \
              as gen_2:
              for x_2 in gen_2:
                assert x_2 is None, \
                  "investment_rules.buy_a_lot_rule: got unexpected plan from when clause 2"
                with engine.prove(rule.rule_base.root_name, 'is_news_positive', context,
                                  (rule.pattern(0),)) \
                  as gen_3:
                  for x_3 in gen_3:
                    assert x_3 is None, \
                      "investment_rules.buy_a_lot_rule: got unexpected plan from when clause 3"
                    rule.rule_base.num_bc_rule_successes += 1
                    yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'is_fundamental_good', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.buy_rule: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'is_technical_bullish', context,
                              (rule.pattern(0),)) \
              as gen_2:
              for x_2 in gen_2:
                assert x_2 is None, \
                  "investment_rules.buy_rule: got unexpected plan from when clause 2"
                rule.rule_base.num_bc_rule_successes += 1
                yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'is_fundamental_good', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.buy_little_rule: got unexpected plan from when clause 1"
            rule.rule_base.num_bc_rule_successes += 1
            yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'is_fundamental_bad', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.sell_rule: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'is_technical_bearish', context,
                              (rule.pattern(0),)) \
              as gen_2:
              for x_2 in gen_2:
                assert x_2 is None, \
                  "investment_rules.sell_rule: got unexpected plan from when clause 2"
                rule.rule_base.num_bc_rule_successes += 1
                yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'is_technical_bearish', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.sell_little_rule: got unexpected plan from when clause 1"
            with engine.prove(rule.rule_base.root_name, 'is_news_negative', context,
                              (rule.pattern(0),)) \
              as gen_2:
              for x_2 in gen_2:
                assert x_2 is None, \
                  "investment_rules.sell_little_rule: got unexpected plan from when clause 2"
                rule.rule_base.num_bc_rule_successes += 1
                yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove(rule.rule_base.root_name, 'is_market_bad', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.hold_market_bad_rule: got unexpected plan from when clause 1"
            rule.rule_base.num_bc_rule_successes += 1
            yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
    ...
    try:
      if all(map(lambda pat, arg:
        ...
        with engine.prove('stock_analysis', 'technical_indicator', context,
          ...
          for x_1 in gen_1:
            assert x_1 is None, \
              "investment_rules.hold_default_rule: got unexpected plan from when clause 1"
            rule.rule_base.num_bc_rule_successes += 1
            yield
        rule.rule_base.num_bc_rule_failures += 1
    finally:
...
def populate(engine):
  This_rule_base = engine.get_create('investment_rules')
...
Krb_filename = '..\\investment_rules.krb'
Krb_lineno_map = (
    ((16, 20), (2, 2)),
    ((22, 28), (4, 4)),
    ((29, 35), (5, 5)),
